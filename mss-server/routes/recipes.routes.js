...
router.get("/all", (req,res,next) => {
	Recipe.find({}).populate("createdBy", ["firstName", "lastName", "profileImg"]).populate("foodType")
	.then(allRecipes => res.status(200).json({message: "All recipes fetched", allRecipes})).catch(error => next(error))
} )

router.post("/create", (req, res, next) => {
	const {imageUrl, name, description, createdBy, instructions, ingredients, prepTime, foodType, servings } = req.body;
	if (name === "") {
	...
	Recipe.findOne({ name })
		.then((recipeFound) => {
			...
			return Recipe.create({imageUrl, name, description, createdBy, instructions, ingredients, prepTime , foodType, servings});
		})	
		.then((createdRecipe) => {
			return FoodType.findByIdAndUpdate(
				foodType, 
...
router.get("/:recipeId", (req, res, next) => {
	...
	Recipe.findById(recipeId).populate("createdBy", ["firstName", "lastName", "profileImg"]).populate({
		path: "foodType",
		populate: { 
			path: "recipes",
...
router.delete("/:recipeId/delete", (req, res, next) => {
	...
	.then((recipeFounded) => {
		FoodType.findByIdAndUpdate(recipeFounded.foodType._id, { $pull: { recipes: recipeFounded._id } },
			{new: true})
	})
